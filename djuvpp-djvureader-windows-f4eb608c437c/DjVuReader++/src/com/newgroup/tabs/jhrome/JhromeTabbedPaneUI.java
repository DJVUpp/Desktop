 ...
 */
package com.newgroup.tabs.jhrome;
...
import com.newgroup.InternalTransferableStore;
import com.newgroup.SwingUtils;
import com.newgroup.tabs.DefaultFloatingTabHandler;
import com.newgroup.tabs.DefaultTabCloseButtonListener;
import com.newgroup.tabs.DefaultTabDropFailureHandler;
import com.newgroup.tabs.DefaultTabFactory;
import com.newgroup.tabs.IFloatingTabHandler;
import com.newgroup.tabs.ITabCloseButtonListener;
import com.newgroup.tabs.ITabDropFailureHandler;
import com.newgroup.tabs.ITabFactory;
import com.newgroup.tabs.ITabbedPaneDndPolicy;
import com.newgroup.tabs.ITabbedPaneWindowFactory;
import com.newgroup.tabs.PropertyGetter;
import com.newgroup.tabs.RecursiveListener;
import com.newgroup.tabs.Tab;
import com.newgroup.tabs.TabDragInfo;
import com.newgroup.tabs.Utils;
import com.newgroup.tabs.event.ITabbedPaneListener;
import com.newgroup.tabs.event.TabAddedEvent;
import com.newgroup.tabs.event.TabMovedEvent;
import com.newgroup.tabs.event.TabRemovedEvent;
import com.newgroup.tabs.event.TabSelectedEvent;
import com.newgroup.tabs.event.TabbedPaneEvent;
import com.newgroup.tabs.event.TabsClearedEvent;
...
public class JhromeTabbedPaneUI extends TabbedPaneUI {
    ...
    private static class TabInfo {
        ...
        /**
         * Whether the tab is being removed (contracting until its width reaches
         * zero, when it will be completely removed)
         */
        boolean isBeingRemoved;

        /**
         * The tab's target x position in virtual coordinate space. It will be
         * scaled down to produce the actual target x position.
         */
        int vTargetX;
        /**
         * The tab's target width in virtual coordinate space. This does not
         * include the overlap area -- it is the distance to the virtual target
         * x position of the next tab. To get the actual target width, this
         * value will be scaled down and the overlap amount will be added.
         */
        int vTargetWidth;

        /**
         * The tab's target bounds in actual coordinate space. Not valid for
         * tabs that are being removed.
         */
        Rectangle targetBounds = new Rectangle();

        /**
         * The tab's current x position in virtual coordinate space. It will be
         * scaled down to produce the actual current x position.
         */
        int vCurrentX;
        /**
         * The tab's current width in virtual coordinate space. This does not
         * include the overlap area -- it is the distance to the virtual current
         * x position of the next tab. To get the actual current width, this
         * value will be scaled down and the overlap amount will be added.
         */
        int vCurrentWidth;
        ...
        /**
         * The x position of the dragging mouse cursor in actual coordinate
         * space.
         */
        int dragX;
        /**
         * The relative x position at which the tab was grabbed, as a proportion
         * of its width (0.0 = left side, 0.5 = middle, 1.0 = right side). This
         * way if the tab width changes while it's being dragged, the layout
         * manager can still give it a reasonable position relative to the mouse
         * cursor.
         */
        double grabX;
    }

    private int overlap = 18;
    ...
    private int maxUniformWidth = 180;
    ...
    /**
     * How many pixels the content panel overlaps the tabs. This is necessary
     * with the Google Chrome appearance to make the selected tab and the
     * content panel look like a contiguous object
     */
    ...
    /**
     * Sets the drop failure handler to a {@link DefaultTabDropFailureHandler}
     * with the given window factory.
     */
    ...
    /**
     * @return whether to make all tabs the same width. If uniform width is not
     * used, the tabs' preferred widths will be used.
     */
    ...
    /**
     * Sets whether to make all tabs the same width. If uniform width is not
     * used, the tabs' preferred widths will be used.
     */
    ...
    /**
     * @return the width to make each tab if {@link #isUseUniformWidth()} is
     * {@code true}. If there are too many tabs in this tabbed pane to fit at
     * this width, they will of course be squashed down.
     */
    ...
    /**
     * Sets the width to make each tab if {@link #isUseUniformWidth()} is
     * {@code true}. If there are too many tabs in this tabbed pane to fit at
     * this width, they will of course be squashed down.
     */
     ...
     *
     * @return the hoverable tab under {@code p}, or {@code null} if no such tab
     * exists.
     *
     ...
     * @param p a {@link Point} in this tabbed pane's coordinate system.
     *
     * @return the draggable tab under {@code p}, or {@code null} if no such tab
     * exists.
     *
     ...
     * @param p a {@link Point} in this tabbed pane's coordinate system.
     *
     * @return the selectable tab under {@code p}, or {@code null} if no such
     * tab exists.
     *
    ...
    /**
     * Inserts a tab at a specific index in the tabbed pane. The new tab will
     * not be selected.
     *
     * @param vIndex the index to add the tab at. Like
     * {@link List#add(int, Object)}, this method will insert the new tab before
     * the tab at {@code index}.
     * @param tab the tab to add.
     *
     * @throws IllegalArgumentException if {@code index} is less than 0 or
     * greater than the tab count, or {@code tab} is already a member of this
     * tabbed pane.
     */
     ...
     * the tab at {@code index}.
     * @param tab the tab to add.
     * @param expand whether to animate the new tab expansion. If {@code false},
     * the new tab will immediately appear at full size.
     *
     * @throws IllegalArgumentException if {@code index} is less than 0 or
     * greater than the tab count, or {@code tab} is already a member of this
     * tabbed pane.
     ...
     * @param vNewIndex the index to move the tab to. Like
     * {@link List#add(int, Object)}, this method will insert {@code tab} before
     * the tab at {@code newIndex}.
     *
     * @throws IllegalArgumentException if {@code newIndex} is less than 0,
     * greater than the tab count, or {@code tab} is not a member of this tabbed
     * pane.
     */
    ...
    /**
     * Moves the given tab with animation (if enabled). Attempting to do this by
     * removing / adding back the tab in the {@link JTabbedPane} won't produce
     * the same effect.
     *
    ...
    /**
     * Removes a tab from this tabbed pane with animation. The layout will
     * animate the tab renderer shrinking before it is actually removed from the
     * component hierarchy. However, the tab will be immediately removed from
     * the list of tabs in this tabbed pane, so {@link #getTabs()}, {@link #addTabInternal(Tab)},
     * {@link #moveTabInternal(Tab, int)}, etc. will all behave as if it is no
     * longer a member of this tabbed pane. If you want to remove the tab and
     * its renderer component immediately, use
     * {@link #removeTabImmediatelyInternal(Tab)}.
     *
     * If the selected tab is removed, one of the adjacent tabs will be selected
     * before it is removed. If it is the only tab, the selection will be
     * cleared before it is removed.
     *
     * {@code tab} the tab to remove. If {@code tab} is not a member of this
     * tabbed pane, this method has no effect.
     */
    ...
    /**
     * Removes a tab from this tabbed pane without animation. This means the tab
     * renderer will be removed from the component hierarchy immediately, unlike
     ...
     * cleared before it is removed.
     *
     * @param tab the tab to remove. If {@code tab} is not a member of this
     * tabbed pane, this method has no effect.
     */
...
//             }
    ...
    /**
     * Removes all tabs from this tabbed pane without animation. This method is
     * equivalent to setting the selected tab to {@code null} and then removing
     * all tabs one by one.
     */
    ...
    /**
     * Notifies this {@code TabbedPane} that a tab's content has changed. If
     * that tab is selected, its old content will be removed from this
     * {@code TabbedPane} and replaced with the tab's new content.
     *
     * @param tab the tab whose content has changed. If {@code tab} is not a
     * member of this tabbed pane, this method has no effect.
     */
    ...
    /**
     * Immediately updates the tabs' bounds to their eventual position where
    ...
    /**
     * @return the new tab button. This tabbed pane manages its appearance and
     * action by default. You are free to modify it however you wish, but
     * behavior is undefined in this case so be careful.
     */
    ...
    /**
     * Removes all tabs, stops animation and attempts to unregister all
     * listeners that could prevent this TabbedPane from being finalized.
     * However, I am not yet certain if this method eliminates all potential
     * memory leaks (excluding references apart from internal TabbedPane code.
     */
    public void dispose() {
        checkEDT();
    ...
    private class TabLayoutManager implements LayoutManager {
        ...
        /**
         * The sustained total width of the tab zone in virtual coordinate
         * space. This does not include the overlap area of the last tab.
         */
        ...
        public void layoutContainer(Container parent) {
            ...
            /**
             * The target x position of the next tab, in virtual coordinate
             * space.
             */
            int vTargetX = 0;

            /**
             * The target width of the tab zone, which is the total target width
             * of all tabs, except those being removed, in virtual coordinate
             * space. This does not include the overlap area of the last tab.
             */
            int vTargetTabZoneWidth = 0;

            /**
             * The current width of the tab zone, which is the total current
             * width of all tabs, including those being removed, in virtual
             * coordinate space. This does not include the overlap area of the
             * last tab.
             */
            ...
            /**
             * The target x position for the right buttons panel, in virtual
             * coordinate space. The logic for this is tricky.
             */
