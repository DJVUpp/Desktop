    ...
    if(djvmDir != null)
    {
      if(size() == 0)
      {
        ...
        for(Enumeration e = files_list.elements(); e.hasMoreElements();)
        {
          ...
          if(file.is_page())
          {
        	String s=file.get_title();
        	s="Page  "+s.substring(1,s.length());
            bookmark.setDisplayName(s);
            bookmark.setObject(new Integer(pageno++));
            addElement(bookmark);
          }
        }
        OutlineTabbedPane.setPagesCount(pageno);
        com.lizardtech.djview.FullBookView.setPagesCount(pageno);
