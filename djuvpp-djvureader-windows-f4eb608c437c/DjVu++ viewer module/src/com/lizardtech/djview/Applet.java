...
/**
 * A class for displaying djvu documents. Very similar to the LizardTech DjVu
 * plugin.
 *
...
public class Applet extends java.applet.Applet implements PropertyChangeListener, Runnable {
	 ...
	 *
	 * @param args
	 *            Should contain the target URL to view.
	 */
		...
		try {
			...
			try {
				...
				if (r.startsWith("jar:")) {
					...
					final String className = (String) attr.getClass()
							.getMethod("getValue", new Class[] { o.getClass() }).invoke(attr, new Object[] { o });
			...
			if (DjVuObject.hasReferences) {
				DjVuOptions.err
						.println(DjVuObject.hasReferences ? "The package has been built without application support."
	 ...
	 *
	 * @throws IOException
	 *             if an error occures decoding the document
	 */
	...
	public String[][] getParameterInfo() {
		final String[][] pinfo = {
				...
				{ "Zoom", "number,width,page", "Initial zoom value False if the toolbar should not be used" },
				{ "ZoomFast", "boolean",
	...
	public URL getURL() {
		...
		if (url == null) {
			try {
				...
				url = retval = (retval == null) ? (new URL(
						"http://www.lizardtech.com/download/files/win/djvuplugin/en_US/welcome.djvu")) : (new URL(
						retval, "index.djvu"));
	...
	public void init() {
		...
		try {
			...
			if ((console == null) && System.getProperty("java.version").startsWith("1.1.")
					&& DjVuBean.stringToBoolean(properties.getProperty("console"), false)) {
				try {
			...
			DjVuOptions.out.println(Applet.class.getName() + " initialized in " + (t / 1000L) + "."
		...
		try {
			...
			if ("status".equalsIgnoreCase(name)) {
				...
				if (object instanceof String) {
					getAppletContext().showDocument(new URL(getURL(), (String) object),
