...
/**
 * <p>
 * DjVuPage decodes single page DjVu files. This class is appropriate for decoding a single page. If
 * an INCL chunk is contained in the page, then either decoding must be done from a URL or
 * createDataPool must be overloaded as in the case of the Document class. The general usage of this
 * class is to create an instance, set any desired values, and then call decode with the relevant
 * data. Decoding may be done either asynchronously or synchronously. By default decoding is
 * synchronous, but this may be changed by calling setAsync(true) before calling decode. If you will
 * be accessing this class from another thread while decoding is in progress then you should use
 * asynchronous mode.
 * </p>
 *
 * <p>
 * In synchronous mode, the decode method will not return until decoding is complete. In
 * asynchronous mode decode returns right away as decoding continues in a separate thread. One
 * method to track asynchronous decode progress is to register a PropertyChangeListener. Each codec
 * will generate a signal when created. If you need to wait for a particular codec, rather than
 * polling, use the waitForCodec method.
 * </p>
 *
 * <p>
 * The updateMap method should be used for progressive rendering. The following example shows how to
 * progressively update an image map in a separate thread.
 * </p>
 * <pre>
...
public class DjVuPage
    ...
    /**
     * This number is incremented for each instance and used for logging. Note that there is no
     * locking done when incrementing, so this number is not guarenteed to be unique.
     */
  ...
  //~ Instance fields --------------------------------------------------------
    ...
    /**
     * All the codec are stored in a hash table to make adding new codecs easier.
     */
    private Hashtable codecTable = new Hashtable();

    /**
     * A reference to the foreground pixmap. This will be regenerated if the if the garbage
  ...
  //~ Methods ----------------------------------------------------------------
    ...
    /**
     * Create a new map. The rectangle of interest and subsample rate are looked up from the old
     * map, and stored in the new map. A null may be returned if the subsample rate is not legal or
    ...
    /**
     * Update an image map based on an existing one. If the existing map is null or anything has
     * been updated, a new image map will be fetched. Otherwise, the existing image will be
     * returned. A null may be returned if the subsample rate is not legal, or no pixels are
     * available.
     *
    ...
    public final GMap updateMap(
        ...
        if (map != null) {
            ...
            if ((subsampleO instanceof Number)
    ...
    /**
     * This thread processes the asynchronous queue. Only one page is decoded at a time, with the
     * most recient requested page processed first.
     */
    public void run() {
...
//          }
    ...
    /**
     * Query the named Codec for this page. If the Codec is not available, and decoding is in
     * progress then wait for it.
     *
     * @param nameLock the name lock of the codec to wait for
     * @param maxTime the maximum amount of time to wait for the specified codec
     *
     * @return the named Codec for this page.
     */
...
//      iff.chunkClose();
    ...
    /**
     * Decode the document from the already opened datapool. Normally this method is called the
     * queue.
     *
     * @throws IOException if an error occurs
     * @throws IllegalStateException if an error occurs
