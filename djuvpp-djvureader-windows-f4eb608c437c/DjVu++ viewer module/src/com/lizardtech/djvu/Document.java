...
public class Document
     ...
     */
    public Document(final URL url) throws IOException {
    ...
    /**
     * Get the respective page and start decoding. If synchronous decoding is used, this call will
     * block until the page is decoded.
     *
     * @param pageno page number to get
     * @param priority decode priority
     * @param dataWait True if bundled pages should be opened even when the data is not ready.
     *
    ...
    /**
     * turns pages storage (caching) on or off
     *
     * @param caching true = on, false = off
     */
    ...
    public CachedInputStream get_data(final String id)
        ...
        final DjVmDir djvmDir = getDjVmDir();
        if (pool == null) {
            // NOTE: the file-list variable is not used.
     ...
     * @throws IOException if an error occurs
     */
    public Document init(final URL url) throws IOException {
        read(url);
    ...
    public void insert_file(
            ...
            int pos)
            throws IOException {
        final DjVmDir.File file = getDjVmDir().createFile(name, id, title, file_type);
        insert_file(file, pool, pos);
    }
...
//      data_pool = data_pool.duplicate(4);
    ...
    public void read(final URL url) throws IOException {
        ...
        setStatus("URL " + url + " initialized      after check for fromat and create it");
    }
